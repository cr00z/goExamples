...
func (s *LaptopServer) RateLaptop(
	...
	for {
		...
		if err != nil {
			return logError(status.Errorf(codes.Internal, "internal error in laptop store: %v", err))
		}
		if found == nil {
		...
		if err != nil {
			return logError(status.Errorf(codes.Internal, "internal error in rating store: %v", err))
		}
